...
export class EntryFormComponent implements OnInit, AfterContentChecked {
  ...
  private loadEntry() {
    if (this.currentAction == "edit") {
      this.route.paramMap.pipe(
        switchMap(params => this.entryService.getById(+params.get("id"))) // colocando '+' faz um cast pra inteiro
      )
        .subscribe(
          (entry) => {
            this.entry = entry
  ...
  private actionsForSuccess(entry: Entry) {
    ...
    this.router.navigateByUrl("entries", { skipLocationChange: true }).then( // promise
